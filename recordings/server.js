 ...
 *
 * Recording search / download / playback backend for the DVR (XVR, WTW OEM).
 *
 * This is a thin, dependency-free Node service (standard library only) that
 * proxies the DVR's netsdk JSON API and its flv.cgi recording stream, then
 * normalizes the results for a browser UI. It coexists with the existing
 * go2rtc live viewer: it listens on its own port and serves its own static
 * UI from recordings/public/.
 *
 * Design notes / known device behavior (see project recon for full detail):
...
const CONFIG = {
  ...
  // Empty password is the DVR default. Override with DVR_PASS. See README.
  ...
  // Max channels the DVR exposes (recon: MAX_CHN = 9).
  maxChannels: parseInt(process.env.DVR_MAX_CHN || '9', 10),
  // Network timeout (ms) for requests to the DVR's JSON API.
  apiTimeoutMs: parseInt(process.env.DVR_API_TIMEOUT_MS || '15000', 10),
  // Idle timeout (ms) for the (potentially long) flv.cgi media stream:
...
/** Stream raw FLV from the DVR straight to the client (passthrough). */
...
/** GET /api/health -- liveness of this service + reachability of the DVR. */
...
/**
 * GET /api/recordings/channels -- channel configuration.
 * Combines /netsdk/GetChannelDetail (per-channel stream specs) so the UI can
 * present a channel picker. Channel index is 0-based; display is +1.
 */
 ...
 * Proxies /netsdk/R.SearchRecord and returns normalized JSON.
 * Optional: &type=Timing,Motion,Alarm,Manual (defaults to all),
 *           &pageSize= &page=
 */
async function handleSearch(req, res, q) {
  ...
  // Accept `types` (UI, comma list) or `type` (API) — same comma syntax.
...
function handleClip(req, res, q) {
  ...
  streamFlvAsMp4(chn, beginUnix, endUnix, res, 'inline', `ch${chn + 1}_${beginUnix}_${endUnix}.mp4`);
}
...
function parseClipParams(q) {
  ...
  if (Number.isNaN(chn) || chn < 0 || chn >= CONFIG.maxChannels) {
...
server.listen(CONFIG.port, CONFIG.bindAddr, () => {
  console.error(
    `[recordings] backend listening on http://${CONFIG.bindAddr}:${CONFIG.port}  ` +
      `-> DVR ${CONFIG.host} (user "${CONFIG.user}", password ${CONFIG.pass ? 'set' : 'EMPTY'})`
  );
  if (!CONFIG.pass) {
    console.error(
      '[recordings] WARNING: DVR password is empty. This is insecure. Set DVR_PASS ' +
        'and change the DVR password. LAN use only.'
    );
  }
